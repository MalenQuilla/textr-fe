...
import {deleteFile, getFiles} from "../services/FilesManService";
...
function Files() {
    ...
    async function fetch(page) {
        ...
        setTimeout(async () => {
            response = await getFiles(page, PaginationConfig.size);
                            ...
                            {files ? files.map((item) => (
                                <div key={item.id} id='outlet-items'>
                                    ...
                                    <div id='outlet-utils'>
                                        ...
                                        <Button id='utils-btn'
                                                onClick={() => deleteAndRefresh(item.id)}>Delete</Button>
                                    </div>
                                </div>
                            )) : <NotFound padding={20}/>}
                        </ul>
